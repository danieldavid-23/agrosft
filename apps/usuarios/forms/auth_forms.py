...
class RegistroTblusuariosForm(forms.Form):  # Cambiar a forms.Form para manejar manualmente los campos
    ...
    def clean_nombres(self):
        """No permite espacios ni caracteres especiales en nombres"""
        nombres = self.cleaned_data.get('nombres', '')
        if not nombres:
            raise forms.ValidationError('El nombre es obligatorio.')
        if ' ' in nombres:
            raise forms.ValidationError('El nombre no puede contener espacios.')
        if not re.match(r'^[a-zA-Zà-ÿ]+$', nombres):
            raise forms.ValidationError('El nombre solo puede contener letras, sin caracteres especiales.')
        return nombres

    def clean_apellidos(self):
        """No permite espacios ni caracteres especiales en apellidos"""
        apellidos = self.cleaned_data.get('apellidos', '')
        if not apellidos:
            raise forms.ValidationError('Los apellidos son obligatorios.')
        if ' ' in apellidos:
            raise forms.ValidationError('Los apellidos no pueden contener espacios.')
        if not re.match(r'^[a-zA-Zà-ÿ]+$', apellidos):
            raise forms.ValidationError('Los apellidos solo pueden contener letras, sin caracteres especiales.')
        return apellidos
    ...
    def clean(self):
        ...
        # Solo validar longitud mínima - se permite cualquier tipo de contraseña (números, letras, símbolos)
    ...
    def clean_correo(self):
        ...
        if correo:
            if Tblusuarios.objects.filter(correo=correo).exists():
                raise forms.ValidationError('⚠️ Este correo electrónico ya está registrado. Por favor inicia sesión o usa otro correo.')
        return correo
